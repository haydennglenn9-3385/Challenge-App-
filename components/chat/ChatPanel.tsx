...
type Message = {
  ...
  reactions?: Reaction[];
  _optimistic?: boolean; // local-only flag, not persisted
};
...
export default function ChatPanel({
  ...
  useEffect(() => {
    ...
    // Challenge & team: simple server-side filter
    if (context.type === "challenge" || context.type === "team") {
      const filter =
    ...
    // DMs: Supabase realtime doesn't support OR filters, so we use two subscriptions —
    // one for messages this user sent, one for messages this user received.
    if (context.type === "dm") {
      const dmUserId = (context as { type: "dm"; userId: string }).userId;

      const sentChannel = supabase
        .channel(`dm-sent-${currentUserId}-${dmUserId}`)
        .on("postgres_changes", {
      ...
      const receivedChannel = supabase
        .channel(`dm-received-${currentUserId}-${dmUserId}`)
        .on("postgres_changes", {
          event: "*", schema: "public", table: "messages",
  ...
  async function handleSend() {
    ...
    // Optimistic: append immediately so the user sees their message instantly
    ...
    if (res.ok) {
      // Reconcile: replace optimistic message with real one from server
      await fetchMessages();
    } else {
                ...
                <div className={`flex flex-col gap-1 max-w-[75%] ${isOwn ? "items-end" : "items-start"}`}>
                  ...
                  {isEditing ? (
                    ...
                    <div className="relative">
                      ...
                      {/* Emoji picker */}
                      {pickerMsgId === msg.id && !msg._optimistic && (
                        <div
                          className={`absolute z-50 bottom-10 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 ${
                  ...
                  {/* Timestamp + edit/delete controls */}
                  <div className={`flex items-center gap-2 px-1 ${isOwn ? "flex-row-reverse" : ""}`}>
                    ...
                    {/* Reaction picker toggle — hidden on optimistic messages */}
                    {!msg._optimistic && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    {isOwn && !isEditing && !msg._optimistic && (
